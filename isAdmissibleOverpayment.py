...
def solution(prices, notes, x):
    ...
    for i in range(items_count):
        overpaying += calculate(prices[i], notes[i])
    print("overpaying: ", overpaying)
    return overpaying <= x


def calculate(price, note):
    if note.startswith("Same"):
        return 0
    else:
        arr = note.split()
        percentage = float(arr[0][0:-1])
        if arr[1] == "higher":
            store_price = round(price / (1 + percentage / 100), 4)
        else:
            store_price = round(price / (1 - percentage / 100), 4)
        print("delta: ", store_price - price)
        return price - store_price